...
function toPounds(val, unit) {
	if (val == null) return null;
	const u = String(unit || '').toLowerCase();
	if (u === 'kg') return val * 2.20462;
	if (u === 'g') return val / 453.592;
...
function classifyTier(weightLb, dim) {
	...
	for (const r of cacheSizeTierRules) {
		const okWeight =
			...
			(weightLb != null && weightLb <= r.shippingWeightMax);
		const okLongest =
			r.longestMax == null ||
			(stats.longest != null && stats.longest <= r.longestMax);
		const okMedian =
			r.medianMax == null ||
			(stats.median != null && stats.median <= r.medianMax);
		const okShortest =
			r.shortestMax == null ||
		...
		if (okWeight && okLongest && okMedian && okShortest && okLG)
...
function applyAdvancedFilters(products) {
	...
	return products.filter((product) => {
		...
		if (currentFilters.fbaFee !== 'all') {
			...
			if (fbaFeeText === 'N/A') return false;
...
async function applyTemplate(templateId) {
	try {
		...
		// Set current filters to template values
		currentFilters = { ...template.filters };

		// Update radio buttons
		Object.keys(currentFilters).forEach((filterKey) => {
			const radio = document.querySelector(
				`input[name="${filterKey}Filter"][value="${currentFilters[filterKey]}"]`
			);
			if (radio) radio.checked = true;
		});
